...
# 导入图像
poslist = os.listdir('./data/INRIADATA/normalized_images/train/pos')
neglist = os.listdir('./data/INRIADATA/normalized_images/train/neg')
testlist = os.listdir('./data/INRIADATA/normalized_images/test/pos')
testnlist = os.listdir('./data/INRIADATA/original_images/test/neg')
# 获得正样本和负样本的HOG特征，并标记
...
for i in range(len(poslist)):
    posimg = io.imread(osp.join('./data/INRIADATA/normalized_images/train/pos', poslist[i]))
...
for i in range(len(neglist)):
    negimg = io.imread(osp.join('./data/INRIADATA/normalized_images/train/neg', neglist[i]))
...
joblib.dump(clf, "./model/trained_svm.m")
...
for i in range(len(testlist)):
    testimg = io.imread(osp.join('./data/INRIADATA/normalized_images/test/pos', testlist[i]))
...
for i in range(len(testnlist)):
    testnegimg = io.imread(osp.join('./data/INRIADATA/original_images/test/neg', testnlist[i]))
...
# 可以导入训练后的SVM
clf = joblib.load("./model/trained_svm.m")
...
plt.savefig('./result/lab3/PR.png', dpi=300)
...
plt.savefig('./result/lab3/ROC.png', dpi=300)
