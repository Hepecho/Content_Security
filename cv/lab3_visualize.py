...
box_list = []
ANN = os.listdir('./data/INRIADATA/INRIAPerson/Train/annotations')
flag = "(Xmax, Ymax)"
for i in range(len(ANN)):
    for line in open('./data/INRIADATA/INRIAPerson/Train/annotations/' + ANN[i], encoding="GBK"):
        if flag in line:
...
imglist = os.listdir('./data/INRIADATA/original_images/test/pos')
for i in range(101, len(imglist)):
    img = io.imread(osp.join('./data/INRIADATA/original_images/test/pos', imglist[i]))
    ...
    cv2.imwrite('./result/lab3/' + imglist[i], img)
    print(str(i))
    if i == 150:
        break
